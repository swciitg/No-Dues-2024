...
const FRONTEND_URL = process.env.REACT_APP_FRONTEND_URL || "http://localhost:3000";
  useEffect(() => {
    ...
    if (rmsg) {
      ...
      if (rchecksum === checksum) {
        ...
        setRedirectUrl(`${FRONTEND_URL}/test/nodues/dashboard`);
      } else {
  ...
  const saveUser = ()=>{
    const email=extractUsername(msg);
    localStorage.setItem('email',`${email}@iitg.ac.in`)
  ...
  return (
    <div>
      ...
      {redirectUrl && <a href={redirectUrl} onClick={saveUser} className=" border px-10 py-3 bg-blue-400 text-white  font-bold my-10 m">Proceed to Dashboard</a>}
   </div> </div>
  );
};
